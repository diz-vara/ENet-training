...
function _bottleneck(internal_scale, use_relu, asymetric, dilated, input, output, downsample)
   ...
   main:add(nn.SpatialConvolution(input, internal, input_stride, input_stride, input_stride, input_stride, 0, 0))
   ...
   elseif asymetric then
      ...
      main:add(nn.SpatialConvolution(internal, internal, asymetric, 1, 1, 1, pad, 0))
      main:add(nn.SpatialConvolution(internal, internal, 1, asymetric, 1, 1, 0, pad))
   elseif dilated then
      main:add(nn.SpatialConvolution(internal, internal, 3, 3, 1, 1, dilated, dilated))
   else
      assert(false, 'You shouldn\'t be here')
   end
   main:add(nn.SpatialBatchNormalization(internal, 1e-3))
   if use_relu then main:add(nn.PReLU(internal)) end
   main:add(nn.SpatialConvolution(internal, output, 1, 1, 1, 1, 0, 0))
...
model = nn.ParallelTable(1, true, true):add(model)
...
loss = nn.CrossEntropyCriterion(classWeights)
